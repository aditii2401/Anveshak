...
export const InvestigateView: React.FC<InvestigateViewProps> = ({
  ...
  const handleSendMessage = (queryToSend?: string) => {
    ...
    setTimeout(() => {
      ...
      if (matchedEntity) {
        if (matchedEntity.id === 'rahul') {
          replyText = `Identity resolved for Rahul Sharma (PID: P-001). He acts as the core coordinating bridge in the Bhopal syndicate. Automated cross-referencing indicates 18 burst calls from 9000010000 across 140 minutes contacting Amit Verma, Suresh Patel, Neeraj Khan, and Vikram Singh. Furthermore, he originated the FIN_001 circular financial transfer loop (₹18,500 sent to AC-002, ₹7,500 returned from AC-003).`;
          followups = [
            'Trace the 18 burst calls from Rahul Sharma',
            'Show the FIN_001 circular fund loop',
          ];
          citations = [
            { label: 'CDR', doc: 'CDR_SPIKE_01 Call Burst Log (18 calls)' },
            { label: 'FIN', doc: 'FIN_001 Circular Ledger (₹18.5k sent / ₹7.5k return)' },
            { label: 'FIR', doc: 'FIR_001 / New Market Delivery Filing' },
          ];
        } else if (matchedEntity.id === 'amit') {
          replyText = `Amit Verma (PID: P-002) is identified as an operating associate and delivery coordinator. He met Rahul K Sharma near New Market (FIR_001) and met Suresh Patel at Habibganj station (FIR_002). Financially, his account AC-002 received ₹18,500 from Rahul Sharma and immediately forwarded ₹12,000 to Suresh Patel.`;
          followups = [
            'Inspect New Market delivery docket (FIR_001)',
            'View Habibganj station meeting log (FIR_002)',
          ];
          citations = [
            { label: 'FIR', doc: 'FIR_001 New Market Meeting' },
            { label: 'FIN', doc: 'FIN_001 Tranche (₹18,500 in, ₹12,000 out)' },
          ];
        } else if (matchedEntity.id === 'suresh') {
          replyText = `Suresh Patel (PID: P-003) is flagged as a circular transfer conduit. His account AC-003 received ₹12,000 from Amit Verma and returned ₹7,500 to Rahul Sharma (AC-001) on 07-Aug-2026, creating a closed cyclic loop indicative of layering.`;
          followups = [
            'Review ₹7,500 return transaction voucher',
            'Check CDR connection with Rahul Sharma',
          ];
          citations = [{ label: 'FIN', doc: 'FIN_001 Return Ledger #4567891203' }];
        } else if (matchedEntity.id === 'neeraj') {
          replyText = `Neeraj Khan (PID: P-004) operates as a regional transporter along the Sehore Road corridor. In FIR_003, he met Rahul Sharma on Sehore Road to exchange transit materials. He also initiated a secondary financial transfer of ₹9,000 to Vikram Singh (FIN_002).`;
          followups = [
            'Inspect Sehore Road vehicle log MP04EF9012',
            'Trace ₹9,000 transfer to Vikram Singh',
          ];
          citations = [
            { label: 'FIR', doc: 'FIR_003 Sehore Road Docket' },
            { label: 'FIN', doc: 'FIN_002 Transfer Voucher' },
          ];
        } else if (matchedEntity.id === 'acct_001') {
          replyText = `Account 4567891201 (AC-001) registered to Rahul Sharma in Bhopal Central is flagged for circular fund routing (FIN_001). It initiated a ₹18,500 transfer to Amit Verma (AC-002), which reached Suresh Patel (AC-003), before ₹7,500 was returned back into AC-001.`;
          followups = [
            'Review full FIN_001 bank statement',
            'Identify all linked beneficiary accounts',
          ];
          citations = [{ label: 'FIN', doc: 'Certified Bank Extract AC-001' }];
        } else {
          replyText = `${matchedEntity.name} (${matchedEntity.pid}) is currently recorded as ${matchedEntity.role}. Multi-source correlation assigns a match confidence of ${(matchedEntity.confidence * 100).toFixed(0)}% across ${matchedEntity.sourcesCount} distinct evidence streams.`;
          followups = [
            `Show all evidence records for ${matchedEntity.name}`,
            `Trace connections linked to ${matchedEntity.name}`,
          ];
        }
      } else if (qLower.includes('call') || qLower.includes('cdr') || qLower.includes('phone')) {
        replyText = `Telecom analysis indicates an anomalous communication burst on 2 Sep 2026 between 18:00 and 23:42. Arjun Rathore and Vikram Patil exchanged 42 calls with an average duration of 48 seconds, indicating rapid operational coordination across cell towers in Nagpur and Bhandara.`;
        followups = [
          'Show cell tower co-location map',
          'Who initiated the first call at 18:02?',
        ];
        citations = [{ label: 'CDR', doc: 'Bilateral Call Log #1182' }];
      } else if (qLower.includes('money') || qLower.includes('transfer') || qLower.includes('18') || qLower.includes('cash')) {
        replyText = `Financial forensics traced ₹18,40,000 originating from a corporate shell, channeled into SBI Account ...9981 held by Vikram Patil, and systematically disbursed in 4 cash withdrawals in Nagpur East and Raipur within 72 hours.`;
        followups = [
          'Audit the ₹18.4L transfer through SBI ...9981',
          'Check Arjun Rathore’s commission payments',
        ];
        citations = [{ label: 'FIN', doc: 'TXN-44S1 Cyclic Ledger' }];
      } else {
        replyText = `Query processed against active docket Case 24/2026. The neural engine identified correlated evidence points across telecommunications and bank records. You can drill down into specific persons of interest or ask to trace funds and call timelines.`;
        followups = [
          'Investigate Arjun Rathore',
          'What is Vikram Patil’s connection to the transfer?',
        ];
      }
