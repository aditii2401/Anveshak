...
export const api = {
  ...
  async getNetworkData(caseId?: string): Promise<NetworkData> {
    ...
    if (USE_BACKEND) {
      try {
        const live = await getLiveNetworkData();
        if (live.nodes.length > 0) return live;
      } catch (err) {
        console.warn('Backend graph unavailable, using demo data:', err);
      }
    }
    ...
    if (USE_BACKEND) {
      try {
        ...
        const live = await getLiveAlerts(
          targetCase?.id ?? 'INV-TRINETRA-001',
          targetCase?.caseNumber ?? ''
        );
        if (live.length > 0) return live;
      } catch (err) {
        console.warn('Backend alerts unavailable, using demo data:', err);
      }
}
